...
void NfbQueue::SetupNdpQueue(unsigned queueId)
{
	...
	// TODO BH FIX: each queue on different DMA and odd ID -- NEW Hack for 400Gbps DYNANIC
        int queues = ndp_get_rx_queue_count(_nfbDevice);
        int blocks = 4; // assumption for 4x100GbE DMA blocks
	int bsize = queues / blocks;

	queueId = (queueId % blocks) * bsize + ((queueId / blocks) * 2 + 1) % bsize;
        printf("QUEUE ID: %u\n", queueId);
